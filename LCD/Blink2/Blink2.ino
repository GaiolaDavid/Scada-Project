...
#include <SoftwareSerial.h> //RS kommunikacio

#define rxPin 12
#define txPin 3
#define enPin 13
#define inverse_logic false //UART
...
void setup()
{
  ...
  IrReceiver.begin(RECV_PIN, ENABLE_LED_FEEDBACK);  // IR vevő indítása (RECV_PIN = 2) (kettes lábon nézzük)
...
void loop()
{
  ...
  char key = keypad.getKey(); //mini billentyűzet
  ...
  if (!master_mod){
  if (key){  //ha a key !=0 (megnyomtuk a master_mod -ot (kilencest) és utána még az 1,2,3 gombokat)
...
void printLCD_master_mod(int ecuNr){ //ecuNr -> melyik ECU küldi az adatot
...
void printLCD_lokal_mod(char key){ //paraméterbe kapom az 1,2,3 gombot
  lcd.clear();
    lcd.setCursor(0, 0);//lcd.setCursor(oszlop,sor)
    Serial.println(key);
    if (key == '1') {  // Ha az "1" gombot nyomják => hőmérséklet, nedvesség, üzemanyag szint
      ...
      lcd.print("Unknown command (not in 1,2,3)");
    }
}

void receiveData() { // soros monitorrol kapom az adatokat
  if (Serial.available()) {
    String received = Serial.readStringUntil('\n');  // Üzenet beolvasása a soros monitorrol a sor végéig
    ...
    if (received[1] == '4'){
      ...
      if (func_nr == 0){ //itt ha func_nr=0 kiíratni a képernyőre
        lcd.clear();
        printLCD_master_mod(ecuNr_int);  //a kapott paraméterek alapján kiír az LCD-re (amikor 0 ás funkció jön akkor a 3. byteba meg kellene kapjam, hogy melyik ECU küldi az adatot)
        //Serial.println(ecuNr_int);
      } else { // itt a func_nr=1, tehát adatokat kell frissítsek (24 bytba megkapom az összes adatot)
...
void data_update(String received){
  ...
  c = received[5] - '0';
  d = received[6] - '0';
  temp = a * 10 + b + (c * 0.1) + (d * 0.01); //új hőmérséklet 
